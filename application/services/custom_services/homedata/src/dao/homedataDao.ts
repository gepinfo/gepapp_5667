...
public async GpSearchForUpdate(homedataData, callback){
    
    new CustomLogger().showLogger('info', 'Enter into homedataDao.ts: GpSearchForUpdate');
    ...
    this.homedata.findOneAndUpdate({ _id: homedataData._id }, homedataData, { new: true }).then((result)	=>	{
        ...
        new CustomLogger().showLogger('info', 'Exit from homedataDao.ts: GpSearchForUpdate');
...
public async GpSearchRelationship(homedataData, callback){
    
    new CustomLogger().showLogger('info', 'Enter into homedataDao.ts: GpSearchRelationship');
    ...
    this.homedata.find({$and: [
        ...
        new CustomLogger().showLogger('info', 'Exit from homedataDao.ts: GpSearchRelationship');

        callback(result);
}).catch((error)=>{
callback(error);
});}
public async GpGetNounCreatedBy(homedataData, callback){
    
    new CustomLogger().showLogger('info', 'Enter into homedataDao.ts: GpGetNounCreatedBy');
    ...
    this.homedata.aggregate(([
                        { $match: { $and: [{ created_by: homedataData.created_by }] } }
                    ])).then((result)	=>	{

        new CustomLogger().showLogger('info', 'Exit from homedataDao.ts: GpGetNounCreatedBy');
